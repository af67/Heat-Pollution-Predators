...
species <- gsub("[0-9]", "", attacks$Species)
...
sum(is.na(attacks$Time)) #this is the only one that still presents 1237 NA. We don't want to delete them all because we would lose 40% of our data. 
table(attacks$Time)#table shows that most of them happen in the afternoon, while 2nd place is owned by morning. To confirm the higher frequency of attacks between 8am and 6pm is this artile (link JC found??)
#We explain this by the fact that, naturally, most of people swim during the day. Therefore, what we do is replacing NA randomly by the proportion of afternoon, morning, evening and night.
...
228/1724
589/1724
#afternoon is 52%, evening is 12%, morning is 34% and night 2%.
...
barplot1 <- barplot(table(attacks$Time),
        col = bar_colors,
        xlab = "Time of Day", ylab = "Frequency of Attacks",
        main = "Frequency of Shark Attacks by Time of Day",
        border = "white",  # White borders for bars
        ylim = c(0, max(table(attacks$Time)) + 5),  # Adjust y-axis limits
        space = 0.5,  # Adjust space between bars
        cex.names = 0.8,  # Adjust font size of axis labels
        font.axis = 2,  # Bold axis labels
        args.legend = list(title = "Time of Day", cex = 0.8),  # Legend settings
        beside = TRUE)
...
## 5.2 RQ2: Is it true that shark attacks have been increasing with climate change?
...
2. \(X_{\text{2}}\)  = CO2 Emissions
3. \(X_{\text{3}}\) = Temperatures Change 
4. \(X_{\text{4}}\) = Sea Level Change
...
str(merged_data3) #Firstly, we run the str function to check if all the variables of interest are num or int, and not chr
