...
merged_map <- merged_map %>%
  mutate(Species_numeric = as.numeric(factor(Species)))

# Get the top 5 species
top_species <- merged_map %>%
  count(Species, sort = TRUE) %>%
  slice_head(n = 5)

# Create a logical condition to include only the top 5 species
condition <- merged_map$Species %in% top_species$Species

# Subset the data based on the condition
filtered_data <- merged_map[condition, ]


#This first regression will be based only on the USA
usa_data <- filtered_data %>%
  filter(Country == "USA")

#correlation matrix
corrusa <- cor(usa_data)

# Run the regression based on the USA and the top 5 species
model1 <- lm(Attackscountry ~ Year + Age + Species + Fatality, data = usa_data)

#log model pour prédire... A FAIRE
...
aus_data <- filtered_data %>%
  filter(Country == "AUSTRALIA")

# Run the regression based on AUSTRALIA and the top 5 species
model2<- lm(Attackscountry ~ Year + Age + Species + Fatality, data = aus_data)
...
model_combined <- lm(Attackscountry ~ Year + Age + Species + Fatality, data = combined_data)
