...
# Assuming there's a common column named "Year" in both datasets
merged_data7 <- merge(attacks_summary, final_ghg_cleaned, by = "Year", all.x = TRUE)

correlation_coefficient <- cor(merged_data7$Number_of_Attacks, merged_data7$`Annual CO2 Emissions`, use = "complete.obs")

# Scatter plot
interactive_scatter_plot_ghg_attacks <- ggplotly(
  ggplot(merged_data7, aes(x = `Annual CO2 Emissions`, y = Number_of_Attacks)) +
    geom_point() +
    geom_smooth(method = "lm", se = FALSE, color = "red") +  # Add a linear trend line
    labs(title = "Relationship Between Sea Level and Shark Attacks",
         x = "GHG emissions",
...
# Display the interactive scatter plot
interactive_scatter_plot_ghg_attacks
